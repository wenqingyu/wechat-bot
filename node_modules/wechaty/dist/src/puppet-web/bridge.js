...
class Bridge {
    constructor(puppet, port) {
        ...
        brolog_env_1.log.verbose('PuppetWebBridge', 'new Bridge({puppet: %s, port: %s})', puppet.constructor.name, port);
    }
    toString() { return `Bridge({puppet: ${this.puppet.constructor.name}, port: ${this.port}})`; }
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            brolog_env_1.log.verbose('PuppetWebBridge', 'init()');
            return this.inject()
                .then(r => {
                ...
                .catch(e => {
                brolog_env_1.log.silly('PuppetWebBridge', 'init() inject() exception: %s', e && e.message || e);
                throw e;
            });
        });
    }
    inject() {
        return __awaiter(this, void 0, void 0, function* () {
            brolog_env_1.log.verbose('PuppetWebBridge', 'inject()');
            try {
                const injectio = this.getInjectio();
                let retObj = yield this.execute(injectio, this.port);
                if (retObj && /^(2|3)/.test(retObj.code)) {
                    brolog_env_1.log.silly('PuppetWebBridge', 'inject() eval(Wechaty) return code[%d] message[%s] port[%d]', retObj.code, retObj.message, retObj.port);
                }
                else {
                ...
                if (retObj && /^(2|3)/.test(retObj.code)) {
                    brolog_env_1.log.silly('PuppetWebBridge', 'inject() Wechaty.init() return code[%d] message[%s] port[%d]', retObj.code, retObj.message, retObj.port);
                }
                else {
                ...
                brolog_env_1.log.silly('PuppetWebBridge', 'inject() ding success');
                return true;
            }
            catch (e) {
                brolog_env_1.log.verbose('PuppetWebBridge', 'inject() exception: %s. stack: %s', e.message, e.stack);
                throw e;
            }
        });
    }
    getInjectio() {
        brolog_env_1.log.verbose('PuppetWebBridge', 'getInjectio()');
        const fs = require('fs');
    ...
    logout() {
        brolog_env_1.log.verbose('PuppetWebBridge', 'quit()');
        return this.proxyWechaty('logout')
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'logout() exception: %s', e.message);
            throw e;
        });
    }
    quit() {
        ...
        return this.proxyWechaty('quit')
            .catch(e => {
            brolog_env_1.log.warn('PuppetWebBridge', 'quit() exception: %s', e && e.message || e);
            // throw e
            /* fail safe */
        });
    }
    getUserName() {
        return __awaiter(this, void 0, void 0, function* () {
            brolog_env_1.log.verbose('PuppetWebBridge', 'getUserName()');
            try {
                const userName = yield this.proxyWechaty('getUserName');
                return userName;
            }
            catch (e) {
                brolog_env_1.log.error('PuppetWebBridge', 'getUserName() exception: %s', e.message);
                throw e;
            }
        });
    }
    contactRemark(contactId, remark) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                brolog_env_1.log.error('PuppetWebBridge', 'contactRemarkAsync() exception: %s', e.message);
                throw e;
            }
        });
    }
    contactFind(filterFunc) {
        return this.proxyWechaty('contactFindAsync', filterFunc)
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'contactFindAsync() exception: %s', e.message);
            throw e;
        });
    }
    roomFind(filterFunc) {
        return this.proxyWechaty('roomFind', filterFunc)
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'roomFind() exception: %s', e.message);
            throw e;
        });
    }
    roomDelMember(roomId, contactId) {
        ...
        return this.proxyWechaty('roomDelMember', roomId, contactId)
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'roomDelMember(%s, %s) exception: %s', roomId, contactId, e.message);
            throw e;
        });
    }
    roomAddMember(roomId, contactId) {
        brolog_env_1.log.verbose('PuppetWebBridge', 'roomAddMember(%s, %s)', roomId, contactId);
        if (!roomId || !contactId) {
            throw new Error('no roomId or contactId');
        }
        return this.proxyWechaty('roomAddMemberAsync', roomId, contactId)
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'roomAddMember(%s, %s) exception: %s', roomId, contactId, e.message);
            throw e;
        });
    }
    roomModTopic(roomId, topic) {
        ...
        return this.proxyWechaty('roomModTopic', roomId, topic)
            .then(_ => topic)
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'roomModTopic(%s, %s) exception: %s', roomId, topic, e.message);
            throw e;
        });
    }
    roomCreate(contactIdList, topic) {
        ...
        return this.proxyWechaty('roomCreateAsync', contactIdList, topic)
            ...
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'roomCreate(%s) exception: %s', contactIdList, e.message);
            throw e;
        });
    }
    verifyUserRequest(contactId, hello) {
        brolog_env_1.log.verbose('PuppetWebBridge', 'verifyUserRequest(%s, %s)', contactId, hello);
        if (!contactId) {
            throw new Error('no valid contactId');
        }
        return this.proxyWechaty('verifyUserRequestAsync', contactId, hello)
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'verifyUserRequest(%s, %s) exception: %s', contactId, hello, e.message);
            throw e;
        });
    }
    verifyUserOk(contactId, ticket) {
        brolog_env_1.log.verbose('PuppetWebBridge', 'verifyUserOk(%s, %s)', contactId, ticket);
        if (!contactId || !ticket) {
            throw new Error('no valid contactId or ticket');
        }
        return this.proxyWechaty('verifyUserOkAsync', contactId, ticket)
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'verifyUserOk(%s, %s) exception: %s', contactId, ticket, e.message);
            throw e;
        });
    }
    send(toUserName, content) {
        ...
        return this.proxyWechaty('send', toUserName, content)
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'send() exception: %s', e.message);
            throw e;
        });
    }
    getMsgImg(id) {
        brolog_env_1.log.verbose('PuppetWebBridge', 'getMsgImg(%s)', id);
        return this.proxyWechaty('getMsgImg', id)
            .catch(e => {
            brolog_env_1.log.silly('PuppetWebBridge', 'proxyWechaty(getMsgImg, %d) exception: %s', id, e.message);
            throw e;
        });
    }
    getMsgEmoticon(id) {
        brolog_env_1.log.verbose('PuppetWebBridge', 'getMsgEmoticon(%s)', id);
        return this.proxyWechaty('getMsgEmoticon', id)
            .catch(e => {
            brolog_env_1.log.silly('PuppetWebBridge', 'proxyWechaty(getMsgEmoticon, %d) exception: %s', id, e.message);
            throw e;
        });
    }
    getMsgVideo(id) {
        return __awaiter(this, void 0, void 0, function* () {
            brolog_env_1.log.verbose('PuppetWebBridge', 'getMsgVideo(%s)', id);
            try {
                return yield this.proxyWechaty('getMsgVideo', id);
            }
            catch (e) {
                brolog_env_1.log.silly('PuppetWebBridge', 'proxyWechaty(getMsgVideo, %d) exception: %s', id, e.message);
                throw e;
            }
        });
    }
    getMsgVoice(id) {
        return __awaiter(this, void 0, void 0, function* () {
            brolog_env_1.log.verbose('PuppetWebBridge', 'getMsgVoice(%s)', id);
            try {
                return yield this.proxyWechaty('getMsgVoice', id);
            }
            catch (e) {
                brolog_env_1.log.silly('PuppetWebBridge', 'proxyWechaty(getMsgVoice, %d) exception: %s', id, e.message);
                throw e;
            }
        });
    }
    getMsgPublicLinkImg(id) {
        return __awaiter(this, void 0, void 0, function* () {
            brolog_env_1.log.verbose('PuppetWebBridge', 'getMsgPublicLinkImg(%s)', id);
            try {
                return yield this.proxyWechaty('getMsgPublicLinkImg', id);
            }
            catch (e) {
                brolog_env_1.log.silly('PuppetWebBridge', 'proxyWechaty(getMsgPublicLinkImg, %d) exception: %s', id, e.message);
                throw e;
            }
        });
    }
    getContact(id) {
        if (id !== id) {
            ...
            brolog_env_1.log.error('PuppetWebBridge', 'getContact(NaN): %s', err);
        ...
        return retryPromise({ max: max, backoff: backoff }, attempt => {
            brolog_env_1.log.silly('PuppetWebBridge', 'getContact() retryPromise: attampt %s/%s time for timeout %s', attempt, max, timeout);
            return this.proxyWechaty('getContact', id)
                ...
                .catch(e => {
                brolog_env_1.log.silly('PuppetWebBridge', 'proxyWechaty(getContact, %s) exception: %s', id, e.message);
                throw e;
            });
        })
            .catch(e => {
            brolog_env_1.log.warn('PuppetWebBridge', 'retryPromise() getContact() finally FAIL: %s', e.message);
            throw e;
        });
        /////////////////////////////////
    }
    ...
    proxyWechaty(wechatyFunc, ...args) {
        return __awaiter(this, void 0, void 0, function* () {
            brolog_env_1.log.verbose('PuppetWebBridge', 'proxyWechaty(%s, %s)', wechatyFunc, args
                ? args.join(', ')
                : '');
    ...
    `.replace(/[\n\s]+/, ' ');
            ...
            try {
                ...
                brolog_env_1.log.warn('PuppetWebBridge', 'proxyWechaty() noWechaty exception: %s', e.stack);
                throw e;
            }
            /**
            ...
            try {
                ...
                brolog_env_1.log.verbose('PuppetWebBridge', 'proxyWechaty(%s, %s) ', wechatyFunc, args.join(', '));
                brolog_env_1.log.warn('PuppetWebBridge', 'proxyWechaty() exception: %s', e.message);
                throw e;
            }
        });
    }
    ...
    execute(script, ...args) {
        return __awaiter(this, void 0, void 0, function* () {
            brolog_env_1.log.silly('PuppetWebBridge', 'execute()');
            ...
            return this.puppet.browser.execute(script, ...args)
                .catch(e => {
                brolog_env_1.log.warn('PuppetWebBridge', 'execute() exception: %s', e.message);
                throw e;
            });
        });
    }
    executeAsync(script, ...args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return this.puppet.browser.executeAsync(script, ...args)
                .catch(e => {
                brolog_env_1.log.warn('PuppetWebBridge', 'executeAsync() exception: %s', e.message);
                throw e;
            });
        });
    }
    ding(data) {
        brolog_env_1.log.verbose('PuppetWebBridge', 'ding(%s)', data);
        return this.proxyWechaty('ding', data)
            .catch(e => {
            brolog_env_1.log.error('PuppetWebBridge', 'ding(%s) exception: %s', data, e.message);
            throw e;
        });
    }
}
