...
ava_1.test('stripHtml()', t => {
    ...
    const strippedHtml = util_lib_1.UtilLib.stripHtml(HTML_BEFORE_STRIP);
...
ava_1.test('unescapeHtml()', t => {
    ...
    const unescapedHtml = util_lib_1.UtilLib.unescapeHtml(HTML_BEFORE_UNESCAPE);
...
ava_1.test('plainText()', t => {
    ...
    const plainText = util_lib_1.UtilLib.plainText(PLAIN_BEFORE);
...
ava_1.test('digestEmoji()', t => {
    ...
    for (let i = 0; i < EMOJI_XML.length; i++) {
        const emojiDigest = util_lib_1.UtilLib.digestEmoji(EMOJI_XML[i]);
...
ava_1.test('unifyEmoji()', t => {
    ...
    ORIGNAL_XML_LIST.forEach(([xmlList, expectedEmojiXml]) => {
        xmlList.forEach(xml => {
            const unifiedXml = util_lib_1.UtilLib.unifyEmoji(xml);
...
ava_1.test('stripEmoji()', t => {
    ...
    EMOJI_STR.forEach(([emojiStr, expectResult]) => {
        const result = util_lib_1.UtilLib.stripEmoji(emojiStr);
        t.is(result, expectResult, 'should strip to the expected str');
    });
    const empty = util_lib_1.UtilLib.stripEmoji(undefined);
...
ava_1.test('downloadStream() for media', t => {
    ...
    server.listen(8000);
    util_lib_1.UtilLib.urlStream('http://127.0.0.1:8000/ding', [{ name: 'life', value: 42 }])
        .then(s => {
        s.on('data', (chunk) => {
...
ava_1.test('getPort() for an available socket port', (t) => __awaiter(this, void 0, void 0, function* () {
    const PORT = 8788;
    let port = yield util_lib_1.UtilLib.getPort(PORT);
    ...
    while (ttl-- > 0) {
        try {
            ...
            port = yield util_lib_1.UtilLib.getPort(PORT);
            server.close();
        }
        catch (e) {
